...
| **9** | **option-mgmt integration (Track B) — yearline side.** **V13.8 adapter:** `to_yearline_context(envelope)` projects the envelope onto the flat, **gated**, versioned `YearlineContext` contract `option-mgmt-2026` consumes (`repair_active`, `distance`/`required_rebound`, `p_retry{10,20,40,60}` preferring the Phase-7 blend + per-horizon `gate_passed`, days-to-touch range, the now-populated `p_success`/`p_successful_reclaim`, `is_stale`, `must_not_auto_execute`), + JSON schema + committed fixtures (gated MSFT + stale/empty) + an export entry point + `ADAPTER_VERSION` pin. Thin deterministic projection — no new modelling; the engine is never imported into option-mgmt's pure core. `OM-Y0…Y5` are separate PRs in `option-mgmt-2026` (read-only here). | ◐ V13.8 DELIVERED | [phase_09/](phase_09/) |
| **11** | **Trend outlook (Track D)** — make the post-confirmation **trend engine** discriminating + (eventually) gated. **TO-0:** handoff coverage (promote above-MA250 names like GOOGL out of `unknown_or_transition` into the trend engine) + thread `distance_to_ma250_pct` + split `indeterminate_trend`. **TO-1:** de-saturated + de-collinearized trend scores via SOTA bounded indicators (Kaufman efficiency ratio, Wilder ADX, trend R², Lo-MacKinlay variance ratio) — `trend_quality` now spreads 0.38–0.76 (was ≈1.0). TO-2…TO-4 (regime probability → forward label → calibration/gate + `trend_outlook_context`) remain. | ◐ TO-0/TO-1 DELIVERED | [phase_11/](phase_11/) |
...
- [`planner/05_trend_outlook_plan.md`](planner/05_trend_outlook_plan.md) — **Track D (MEDIUM):** trend-engine outlook TO-0…TO-4 → delivered as **`phase_11/`** (TO-0/TO-1 done; grounded in `research/03`+`04`).
